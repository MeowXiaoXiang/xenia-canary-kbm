...
def generate_version_h(build_dir="build"):
    ...
    if git_is_repository():
        ...
        (upstream_commit, upstream_commit_short) = git_get_upstream_base_info()
...
#define XE_BUILD_UPSTREAM_COMMIT "{upstream_commit}"
#define XE_BUILD_UPSTREAM_COMMIT_SHORT "{upstream_commit_short}"
...
def git_get_upstream_base_info():
    """Returns the upstream revision actually integrated into this build."""
    upstream_ref = "upstream/canary_experimental"
    p = subprocess.Popen(
    ...
    commit = stdout.decode("ascii").strip()
    if p.returncode or not commit:
    ...
    (stdout, _) = p.communicate()
    return commit, stdout.decode("ascii").strip() or "unknown"
