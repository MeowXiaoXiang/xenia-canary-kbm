...
void ImGuiDrawer::AddDialog(ImGuiDialog* dialog) {
  ...
  if (input_system_) {
    input_system_->SetHostUIVisible(true);
  }
}
...
void ImGuiDrawer::RemoveDialog(ImGuiDialog* dialog) {
  ...
  dialogs_.erase(it);
  if (dialogs_.empty() && input_system_) {
    input_system_->SetHostUIVisible(false);
  }
...
void ImGuiDrawer::LoadInputSystem(hid::InputSystem* input_system) {
  ...
  if (input_system_) {
    input_system_->SetHostUIVisible(!dialogs_.empty());
  }
}
...
bool ImGuiDrawer::LoadTraditionalChineseFont(ImGuiIO& io, float font_size) {
#if XE_PLATFORM_WIN32
  PWSTR fonts_dir;
  HRESULT result = SHGetKnownFolderPath(FOLDERID_Fonts, 0, NULL, &fonts_dir);
  if (FAILED(result)) {
    XELOGW("Unable to find Windows fonts directory for Traditional Chinese");
    return false;
  }

  std::filesystem::path font_path = std::wstring(fonts_dir);
  CoTaskMemFree(static_cast<void*>(fonts_dir));
  font_path.append("msjh.ttc");
  if (!std::filesystem::exists(font_path)) {
    XELOGW(
        "Unable to load Microsoft JhengHei; Traditional Chinese UI "
        "characters will be boxes");
    return false;
  }
  ...
  static constexpr char kUiGlyphs[] =
      ...
      "變更且先再使用版本尚會立即無法保留目前控制器略過請後以它取代"
      "導覽方向左右上下搖桿按鈕扳機返回開始肩"
  ...
  if (glyph_ranges.empty()) {
    ...
    builder.AddText(kUiGlyphs);
    builder.BuildRanges(&glyph_ranges);
  }

  ImFontConfig zh_font_config;
  zh_font_config.MergeMode = true;
  zh_font_config.OversampleH = zh_font_config.OversampleV = 2;
  zh_font_config.PixelSnapH = true;
  zh_font_config.FontNo = 0;
  io.Fonts->AddFontFromFileTTF(xe::path_to_utf8(font_path).c_str(), font_size,
                               &zh_font_config, glyph_ranges.Data);
  return true;
#else
  return false;
#endif
...
void ImGuiDrawer::InitializeFonts(const float font_size) {
  ...
  // Load the compact Traditional Chinese set first. The Japanese range also
  // contains many Han characters, and ImGui keeps the first merged glyph for a
  // codepoint. Reversing this order would mix MS Gothic and Microsoft JhengHei
  // in the same Traditional Chinese sentence.
  LoadTraditionalChineseFont(io, font_size);
