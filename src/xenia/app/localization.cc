...
constexpr std::array<Translation, static_cast<size_t>(StringId::kCount)>
    kTranslations = {{
        ...
        {"Mouse response curve (default 0.8; 1.0 is linear):",
         "滑鼠反應曲線（預設 0.8；1.0 為線性）："},
        {"0.8 boosts fine aim. Above 1.0 slows small movements; 1.0 is linear.",
         "0.8 會提升細微瞄準的輸出；高於 1.0 會降低小幅移動；1.0 為線性。"},
        ...
        {"Save settings and close", "儲存設定並關閉"},
        {"Settings file: kbm.toml", "設定檔：kbm.toml"},
        {"Changes are applied but not saved.", "變更已套用，但尚未儲存。"},
        ...
        {"This kbm.toml uses an incompatible schema and was not loaded. Use "
         ...
         "此 kbm.toml 使用不相容的版本，尚未載入。請先還原預設值，再儲存設定以取代它。"},
        {"Unbound", "未綁定"},
        {"Bind Escape", "綁定 Esc"},
        {"Cancel capture", "取消擷取"},
        ...
        {"Save settings and start 60-second capture",
         "儲存設定並開始 60 秒取樣"},
        {"Stop and save report", "停止並儲存報告"},
        {"Cancel sampling", "取消取樣"},
        {"Starting a capture closes this window so KBM input can resume. "
         "Unsaved settings are saved first.",
         "開始取樣後會關閉此視窗，讓 KBM 輸入恢復；未儲存的設定會先儲存。"},
